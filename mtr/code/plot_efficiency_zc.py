...
def get_box(data, index, label, epsilon="0.1"):#index  =hypercube
...
def plot_interval_size1(data_independent, data_gumbel, data_empirical, model, name):
    ...
    plt.show()
    plt.savefig(base_path / ("ZC_%s_Hyperrectangle_box_plot_%s.eps" % (model, name)), format='eps')
    plt.close()
...
def plot_interval_size_both(data_independent_mlp, data_gumbel_mlp, data_empirical_mlp,
    ...
    plt.show()
    plt.savefig(base_path / ("ZC_Hyperrectangle_box_plot_%s.eps" % (name)), format='eps')
    plt.close()
...
def plot_interval_size_empirical(data_1, data_2, data_3, model, name):
    ...
    plt.show()
    plt.savefig(base_path / ("ZC_%s_all_empirical_Hyperrectangle_box_plot_%s.eps" % (model, name)), format='eps')
    plt.close()
...
def plot_interval_size_gumbel(data_1, data_2, data_3, model, name):
    ...
    plt.show()
    plt.savefig(base_path / ("ZC_%s_all_gumbel_Hyperrectangle_box_plot_%s.eps" % (model, name)), format='eps')
    plt.close()

def plot_interval_size(data_gumbel_mlp, data_empirical_mlp, data_gumbel_rf, data_empirical_rf, model, name):
    medianprops = dict(linestyle='-', linewidth=3, color='firebrick')
    columns_len = len(config["targets"])
    epsilon = str(1 - pow(1 - 0.1, 1 / columns_len))
    #columns = list(data_independent_mlp["AVG"][epsilon]["AVG"])[:-1]
    ...
    boxes = [
        #get_box(data_independent, s_i, label="Independent Copula", epsilon=epsilon),
        get_box(data_empirical_rf, s_i, label="Empirical Copula RF", epsilon='0.1'),
        get_box(data_empirical_mlp, s_i, label="Empirical Copula NN", epsilon='0.1'),
    ]
    axs.bxp(boxes, medianprops=medianprops)
    axs.set_title("%s : Hyper-rectangle median volume for %s" % (model, name))
    #plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cal_size", default=0.01, type=float)
    ...
    name = Path(args.name)
    path_independent_mlp = str(cal_size) + "_results_independent_nn_conf_interval.json"
    path_gumbel_mlp = str(cal_size) + "_results_gumbel_nn_conf_interval.json"
    path_empirical_mlp = str(cal_size) + "_results_empirical_nn_conf_interval.json"
    config_path = "config.json"

    with open(base_path / config_path) as f:
        config = json.load(f)

    with open(base_path / path_independent_mlp, "r") as f:
        data_independent_mlp = json.load(f)

    with open(base_path / path_gumbel_mlp, "r") as f:
        data_gumbel_mlp = json.load(f)

    with open(base_path / path_empirical_mlp, "r") as f:
        data_empirical_mlp = json.load(f)

    #plot_interval_size1(data_independent_mlp, data_gumbel_mlp, data_empirical_mlp, "NN", name)

    path_independent_rf = str(cal_size) + "_results_independent_rf_conf_interval.json"
    path_gumbel_rf = str(cal_size) + "_results_gumbel_rf_conf_interval.json"
    path_empirical_rf = str(cal_size) + "_results_empirical_rf_conf_interval.json"
    ...
    #plot_interval_size1(data_independent_rf, data_gumbel_rf, data_empirical_rf, "NN", name)
    plot_interval_size(data_gumbel_mlp, data_empirical_mlp, data_gumbel_rf, data_empirical_rf, "RF & NN", name)
