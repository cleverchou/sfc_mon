...
func updateContainerStat() {
	...
	wg.Wait()
	pushInfluxdb(containerData.Res)
	dat, err := json.Marshal(containerData.Res)
        fmt.Print(containerData.Res)
	if err != nil {
...
/*
	...
	fmt.Print(ms.Metric)
	fmt.Print(ms.Value)
	fmt.Print(ms.TagsMap["dockername"]+"\n")
*/

func pushInfluxdb(metrics []Metric) {
	...
	client := influxdb2.NewClient("http://influxServiceHost:8086", fmt.Sprintf("%s:%s","",""))
	...
	writeAPI := client.WriteAPIBlocking("", "netmonitor/autogen")
	// create point using full params constructor
	for _ , ms := range metrics {
		switch v := ms.ValueUntyped.(type){
                case int :
                    p := influxdb2.NewPoint(ms.Metric,
                         map[string]string{"sfc": "sfc1"},
		         map[string]interface{}{"Value": ms.ValueUntyped.(int), "docker": ms.TagsMap["dockername"]},
		         time.Now())
	        ...
	        case uint64 :
                    p := influxdb2.NewPoint(ms.Metric,
                         map[string]string{"sfc": "sfc1"},
                         map[string]interface{}{"Value": strconv.FormatUint(ms.ValueUntyped.(uint64),10), "docker": ms.TagsMap["dockername"]},
                         time.Now())
                    err := writeAPI.WritePoint(context.Background(), p)
                    if err != nil {
                        logger.Errorf("Write error:", err)
                ...
                case float64:
                    p := influxdb2.NewPoint(ms.Metric,
                         map[string]string{"sfc": "sfc1"},
		            map[string]interface{}{"Value": ms.ValueUntyped.(float64), "docker": ms.TagsMap["dockername"]},
		                time.Now())
                    err := writeAPI.WritePoint(context.Background(), p)
	                if err != nil {
                            logger.Errorf("Write error:", err)
			...
			//2021-07-10  no need to push mysql 
			//if strings.EqualFold(ms.Metric, "cpu.util") {
		        //    pushMySQL(1, time.Now().Unix(), ms.ValueUntyped.(float64), ms.TagsMap["dockername"])
		        //} else if strings.EqualFold(ms.Metric, "mem.bytes.used.percent") {
		        //    pushMySQL(2, time.Now().Unix(), ms.ValueUntyped.(float64), ms.TagsMap["dockername"])
	               // }
                default :
                    fmt.Printf("type %s \n",v)
	    }
...
func Poolmysql(){
	var err error
	db, err = gorm.Open(mysql.Open("root:123456@tcp(mysqlServiceHost:3306)/record_cloud?charset=utf8&parseTime=True&loc=Local"),
                &gorm.Config{})
        if err != nil {
                logger.Errorf("open mysql ERROR:", err)
                return
        }
...
func pushMySQL(table int, time int64, value float64, docker string) {
	fmt.Println(table, time, value, docker)
//	db, err = gorm.Open(mysql.Open("root:root@tcp(mysqlServiceHost:3306)/perf?charset=utf8&parseTime=True&loc=Local"),
//		&gorm.Config{})
	//defer db.Close()
//	if err != nil {
//		logger.Errorf("open mysql ERROR:", err)
...
//	}
	...
	case 1:
		...
		if err1 := db.Create(cpu).Error; err1 != nil {
			logger.Errorf("Write mySQL error for CPU:", err1)
			return
		}
	case 2:
		mem := &MEM{
		...
		if err2 := db.Create(mem).Error; err2 != nil {
			logger.Errorf("Write mySQL error for MEM:", err2)
			return
		}
	}
}
...
func getInspect(containerId string) (containerInspect *ContainerInspect, err error) {
	...
	}*/containerInspect.Nid = "dockerMon"
	...
	if config.G.Identify == "hostname" {
		...
		for i := 0; i < len(envs); i++ {
			if strings.Contains(envs[i], "NAME") {
	...
	if cpuPeriod != 0 {
		containerInspect.CpuQuota = float64(inspect.HostConfig.CPUQuota) / float64(cpuPeriod)
	} else {
...
func toDockerMetric(name, nid, endpoint string, value interface{}, tags map[string]string) Metric {
	...
	ret.TagsMap["dockername"] = endpoint
	return ret
}
